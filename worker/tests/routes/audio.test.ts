...
beforeAll(async () => {
	await env.R2_BUCKET.put(
		`books/${AUTHOR}/${TITLE}/audio/${RENDITION}/builds/${BUILD}/chapter-01.m4a`,
		FAKE_AUDIO,
	);
});
...
describe("GET /api/v1/books/:author/:title/audio/:chapter", () => {
	it("streams audio with correct content type", async () => {
		const res = await app.request(`/api/v1/books/${AUTHOR}/${TITLE}/audio/01?${QUERY}`, {}, env);
	...
	it("requires rendition and build query params", async () => {
		const res = await app.request(`/api/v1/books/${AUTHOR}/${TITLE}/audio/01`, {}, env);
	...
	it("returns 404 for missing chapter", async () => {
		const res = await app.request(`/api/v1/books/${AUTHOR}/${TITLE}/audio/99?${QUERY}`, {}, env);
		expect(res.status).toBe(404);
		await res.arrayBuffer();
	});

	it("returns 400 for invalid slug", async () => {
		const res = await app.request(`/api/v1/books/INVALID/foo/audio/01?${QUERY}`, {}, env);
		expect(res.status).toBe(400);
		await res.arrayBuffer();
	});

	it("returns 400 for invalid chapter format", async () => {
		const res = await app.request(`/api/v1/books/${AUTHOR}/${TITLE}/audio/abc?${QUERY}`, {}, env);
		expect(res.status).toBe(400);
		await res.arrayBuffer();
	});
	...
	it("handles range requests with 206", async () => {
		const res = await app.request(
			`/api/v1/books/${AUTHOR}/${TITLE}/audio/01?${QUERY}`,
	...
	it("zero-pads single digit chapter", async () => {
		const res = await app.request(`/api/v1/books/${AUTHOR}/${TITLE}/audio/1?${QUERY}`, {}, env);
		expect(res.status).toBe(200);
		await res.arrayBuffer();
	});
});
