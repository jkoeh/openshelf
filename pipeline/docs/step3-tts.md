...
2. Trims the prefix audio using Kokoro's segment grapheme boundaries — each `pipeline()` call returns `(graphemes, phonemes, audio)` tuples, and we accumulate grapheme text until it covers the prefix, then keep only subsequent segments
...
This means `chunk_audio_starts` points to where the **real content** (post-trim) begins, not the prefix. Word alignment receives original chunk texts without prefixes.
...
- `_split_segments_at_prefix(results, context_prefix, sample_rate, fade_ms)` — trims prefix audio using segment graphemes
